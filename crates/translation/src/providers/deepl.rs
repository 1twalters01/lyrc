...
impl DeepLTranslator {
    async fn translate_cues(
        ...
        Python::attach(|py| -> PyResult<_> {
            ...
            let provider_module = PyModule::import(py, "translator.providers.deepl.provider")?;
            let options_module = PyModule::import(py, "translator.providers.deepl.options")?;
            ...
            let options = options_module.getattr("DeepLOptions")?.call1(())?;

            let deepl_translator = provider_module.getattr("DeepLTranslator")?.call0()?;

            let providers = PyDict::new(py);
            providers.set_item("deepl", deepl_translator)?;
            ...
            let coroutine = translation_service.call_method1(
                "translate",
                ("deepl", lrc_contents, to_language_py, options),
            )?;
